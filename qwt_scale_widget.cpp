...
void QwtScaleWidget::draw( QPainter *painter ) const
{
    ...
    if ( d_data->scaleDraw->orientation() == Qt::Horizontal )
    {
        ...
        r.adjust(0, 0, -1, 0); // -1px padding for last tick
    }
    else
    {
        r.setTop( r.top() + d_data->borderDist[0] );
        r.setHeight( r.height() - d_data->borderDist[1] );
        r.adjust(0, 0, 0, -1); // -1px padding for last tick
    }
...
{
    int bd0 = d_data->borderDist[0];
    int bd1 = d_data->borderDist[1];
    ...
    length -= 1; // -1px padding for last tick
...
QSize QwtScaleWidget::minimumSizeHint() const
{
    ...
    int length = 0;
    length += qMax( 0, d_data->borderDist[0] );
    length += qMax( 0, d_data->borderDist[1] );
...
void QwtScaleWidget::setColorBarEnabled( bool on )
{
    ...
    {
        if (on && !d_data->colorBar.colorMap)
