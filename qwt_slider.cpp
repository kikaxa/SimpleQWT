...
{
    initSlider( orientation, scalePos );
}
    ...
    ScalePos scalePos )
{
    ...
    d_data->handleSize.setWidth( handleThickness );
    d_data->handleSize.setHeight( 2 * handleThickness );
...
{
    return scaleDraw->scaleMap().transform( value );
}
...
{
    return scaleDraw->scaleMap().invTransform(
...
*/
bool QwtSlider::getScrollMode( const QPoint &p ) const
{
...
QSize QwtSlider::minimumSizeHint() const
{
    ...
    if ( d_data->scalePos != QwtSlider::NoScale )
    {
        const int sdLength = 0;//scaleDraw->minLength( font() );

        int l = sdLength + handleLength;
